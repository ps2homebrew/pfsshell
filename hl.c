...
int
copyto (const char *mount_point, const char *dest, const char *src)
{
  int retval = 0;
...
int
copyfrom (const char *mount_point, const char *src, const char *dest)
{
  int retval = 0;
  int out_file = open (dest, O_CREAT | O_WRONLY, 0664);
  if (out_file != -1)
    {
      int result = iomanx_mount ("pfs0:", mount_point, 0, NULL, 0);
      if (result >= 0)
  ...
  while ((result = iomanx_dread (dh, &dirent)) && result != -1)
    {
      ...
      switch (m & FIO_S_IFMT)
	{ /* item type */
	case FIO_S_IFLNK: mode[0] = 'l'; break;
	case FIO_S_IFREG: mode[0] = '-'; break;
	case FIO_S_IFDIR: mode[0] = 'd'; break;
...
/* list files at PFS */
int
ls (const char *mount_point, const char *path)
{
  int retval = 0;
  int result = iomanx_mount ("pfs0:", mount_point, 0, NULL, 0);
  if (result >= 0)
...
int
mkpart (const char *mount_point, long size_in_mb, int format)
{
  char tmp[256];
  if (size_in_mb >= 1024)

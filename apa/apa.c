...
void apaSaveError(u32 device, void *buffer, u32 lba, u32 err_lba)
{
	memset(buffer, 0, 512);
	*(u32 *)buffer=err_lba;
	atadDmaTransfer(device, buffer, lba, 1, ATA_DIR_WRITE);
...
int getPartErrorSector(u32 device, u32 lba, int *lba_out)
{
	...
	if(atadDmaTransfer(device, clink->header, lba, 1, ATA_DIR_READ))
		return -EIO;
	...
	if(clink==NULL && (mode & O_CREAT))
	{
		if((rv=apaCheckPartitionMax(device, params->size))>=0) {
			if((clink=apaAddPartitionHere(device, params, emptyBlocks, sector, &rv))!=NULL)
			{
				...
				atadDmaTransfer(device, clink2->header, sector+8     , 2, ATA_DIR_WRITE);
				atadDmaTransfer(device, clink2->header, sector+0x2000, 2, ATA_DIR_WRITE);
				cacheAdd(clink2);
			}
		}
	}
...
{
	if(atadDmaTransfer(device, header, lba, 2, ATA_DIR_READ)!=0)
		return -EIO;
...
{
	if(atadDmaTransfer(device, header, lba, 2, ATA_DIR_WRITE))
		return -EIO;
	return 0;
}
	...
	*format=0;
	if((rv=apaReadHeader(device, clink->header, 0))==0)
	{
		*format=clink->header->mbr.version;
		if(atadDmaTransfer(device, clink->header, APA_SECTOR_SECTOR_ERROR, 2, ATA_DIR_READ))
			rv=-EIO; // return -EIO;
		if(rv==0){

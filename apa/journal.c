...
{// this write any thing that in are journal buffer :)
	if(atadFlushCache(device))
		return -EIO;
	if(atadDmaTransfer(device, &journalBuf, APA_SECTOR_APAL, 1, ATA_DIR_WRITE))
		return -EIO;
	if(atadFlushCache(device))
		return -EIO;
	return 0;
...
{	// copys apa headers from apal to apa system
	...
	if(atadDmaTransfer(device, &journalBuf, APA_SECTOR_APAL, sizeof(apa_journal_t)/512, ATA_DIR_READ)){
		journalReset(device);
		return -EIO;
	}
	if(journalBuf.magic==APAL_MAGIC)
	{
		...
		{
			if(atadDmaTransfer(device, clink->header, sector, 2, ATA_DIR_READ))
				break;
			if(atadDmaTransfer(device, clink->header, journalBuf.sectors[i], 2, ATA_DIR_WRITE))
				break;
		}
		cacheAdd(clink);

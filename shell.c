...
static int shell_loop(FILE *in, FILE *out, FILE *err,
	...
	while (!interrupt &&
				 (fprintf(out, "%s", prompt), fgets(line, sizeof(line), in)) &&
				 memcmp(line, "exit", 4) && memcmp(line, "quit", 4) &&
				 memcmp(line, "bye", 3)) {
		char *tokens[256];
		size_t count;
		...
		if (ctx->setup) {
			if (ctx->mount)
				sprintf(prompt, "%s:%s# ", strchr(ctx->mount_point, ':') + 1,
								ctx->path);
			else
				sprintf(prompt, "# ");
		}
...
static int do_cd(context_t *ctx, int argc, char *argv[]) {
	...
	if (!strcmp(argv[1], "/")) /* root */
		...
		if (strlen(ctx->path) > 1) strcat(ctx->path, "/");
...
static int do_get(context_t *ctx, int argc, char *argv[]) {
	...
	if (in >= 0) {
		...
		if (out != -1) {
			char buf[4096];
			ssize_t len;
			while (!interrupt && (len = iomanx_read(in, buf, sizeof(buf))) > 0) {
...
static int do_put(context_t *ctx, int argc, char *argv[]) {
	...
	if (in != -1) {
		...
		if (out >= 0) {
			...
			ssize_t len;
			while (!interrupt && (len = read(in, buf, sizeof(buf))) > 0) {
...
static void interrupt_proc(int sig) {
...
int shell(FILE *in, FILE *out, FILE *err) {
	...
	if (ctx.mount) do_umount(&ctx, 0, NULL);
	atad_close();
	return (result);
}
